$(document).ready(function() {
    ...
    $('.toleft').click(function() {
        $('.history-cars ul').animate({ right: '+=1097px' }, "slow");

    });
    $('.toright').click(function() {
        $('.history-cars ul').animate({ left: '-=1097px' }, "slow");

    });

    var phonenum = /^\d{3}-\d{8}|\d{4}-\d{7}$/;
    $('.sales-input-box button').click(function() {
        if (phonenum.test(this.value)) {
